...
const CourseDetails = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row w-full min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      {/* Sidebar */}
      <div className="w-full md:w-96 bg-white shadow-xl border-r border-gray-200 flex flex-col">
        <div className="p-4 md:p-6 border-b border-gray-200 bg-gradient-to-r from-indigo-600 to-purple-600 text-white">
          <div className="flex items-center justify-start gap-2 mb-2">
            <div>
              <ArrowLeft className="w-5 h-5 cursor-pointer hover:bg-white/20 rounded p-0.5" />
            </div>
            <h2 className="text-base md:text-lg font-bold leading-tight">{course.title}</h2>
          </div>
          ...
          {course.modules.map((module, index) => (
            ...
            >
              <div className="flex items-center">
                <button
                  onClick={() => toggleModule(module._id)}
                  className="flex-1 px-4 md:px-6 py-3 md:py-4 flex justify-between items-center hover:bg-gray-50"
                >
                  <div className="flex-1 text-left">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[10px] md:text-xs bg-indigo-100 text-indigo-600 px-2 py-1 rounded-full font-medium">
                        Module {index + 1}
                      </span>

                      {module.deleted && (
                        <span className="text-[10px] md:text-xs bg-red-100 text-red-600 px-2 py-1 rounded-full font-medium">
                          Deleted
                        </span>
                      )}
                    </div>
              ...
              {expandedModules.includes(module._id) &&
                module.lessons.map((lesson, lessonIndex) => (
                  <div key={lesson._id} className="flex items-center">
                    <button
                      onClick={() => setSelectedLesson(lesson)}
                      className={`flex-1 text-left px-6 py-3 hover:bg-white border-l-4 ${
                        selectedLesson?._id === lesson._id
                          ? "bg-white border-indigo-500"
                          : "border-transparent"
                      }`}
          ...
          <div className="space-y-4 md:space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 md:p-6">
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4 gap-3">
                <h2 className="text-lg md:text-2xl font-bold">{selectedLesson.title}</h2>
                <div className="flex items-center gap-2">
                  {selectedLesson.isCompleted && (
                  ...
                  <button className="bg-indigo-100 hover:bg-indigo-200 text-indigo-600 px-3 md:px-4 py-1.5 md:py-2 rounded-lg text-xs md:text-sm flex items-center gap-2">
                    <Download className="w-4 h-4" />
                    Resources
                  </button>
                </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            <div className="mb-6 md:mb-8">
              <img
                src={course.thumbnail}
                alt={course.title}
                className="w-64 md:w-96 h-40 md:h-56 object-cover rounded-xl shadow-lg mb-4 md:mb-6"
              />
            </div>
